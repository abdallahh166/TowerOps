...
public sealed class ForgotPasswordCommandHandler : IRequestHandler<ForgotPasswordCommand, Result>
{
    ...
    public async Task<Result> Handle(ForgotPasswordCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var body = $"<p>Your TowerOps OTP is <b>{otp}</b>.</p><p>It expires in 15 minutes.</p>";
            await _emailService.SendEmailAsync(email, "TowerOps Password Reset OTP", body, cancellationToken);
        }
        catch
        {
