...
namespace TowerOps.Application.Commands.Reports.ExportChecklist;
...
public sealed class ExportChecklistCommandHandler : IRequestHandler<ExportChecklistCommand, Result<byte[]>>
{
    ...
    private async Task<IReadOnlyList<TowerOps.Domain.Entities.UnusedAssets.UnusedAsset>> LoadUnusedAssetsAsync(
        IReadOnlyList<Visit> visits,
        CancellationToken cancellationToken)
    {
        if (_unusedAssetRepository is null || visits.Count == 0)
            return Array.Empty<TowerOps.Domain.Entities.UnusedAssets.UnusedAsset>();
    ...
    private static void BuildUnusedAssetsSheet(
        XLWorkbook workbook,
        IReadOnlyList<Visit> visits,
        IReadOnlyList<TowerOps.Domain.Entities.UnusedAssets.UnusedAsset> unusedAssets)
    {
