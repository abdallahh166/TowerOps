...
public class CreateUserCommandHandler : IRequestHandler<CreateUserCommand, Result<UserDto>>
{
    ...
    public async Task<Result<UserDto>> Handle(CreateUserCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                var emailBody =
                    $"<p>Welcome to TowerOps.</p><p>Email: <b>{request.Email}</b></p><p>Temporary Password: <b>{temporaryPassword}</b></p><p>Please login and change your password immediately.</p>";
                await _emailService.SendEmailAsync(request.Email, "TowerOps User Invitation", emailBody, cancellationToken);
            }
            catch
            {
